...
class Particle:
    ...
    def compute_fitness(self, image_sizes, paper_size, scaling_penalty_factor=1, overlap_penalty_factor=1):
        ...
        for i, (x, y, scale) in enumerate(positions):
            ...
            resizing_deviation = abs(scale - avg_scale) ** 2  # Using square of the deviation from the average scale
            ...
            for j, (x2, y2, scale2) in enumerate(positions):
                if i != j:
                    ...
                    if (x + new_width > paper_size[0] or y + new_height > paper_size[1]):
                        ...
                        overlapping_area += out_of_bounds_area

        # Compute the uncovered area
        uncovered_area = total_area - sum_image_areas + overlapping_area
    ...
    def run(self):
        ...
        for _ in range(self.iterations):
            for particle in self.population:
                fitness = particle.compute_fitness(image_sizes, paper_size, scaling_penalty_factor=200, overlap_penalty_factor=5)
